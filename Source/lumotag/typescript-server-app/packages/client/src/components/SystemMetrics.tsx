...
export const SystemMetrics: React.FC = () => {
  const {
    ...
    error,
    isError,
  } = useQuery<SystemStatus>("systemMetrics", apiService.getStatus, {
  ...
  const requestData = [
    ...
    { name: "Errors", value: metrics?.metrics.errorCount || 0 },
    {
      name: "Success Rate",
      value: metrics
        ? ((metrics.metrics.requestCount - metrics.metrics.errorCount) /
            Math.max(metrics.metrics.requestCount, 1)) *
          100
        : 0,
    },
  ];
      ...
      <div className="metrics-content">
        ...
        <div className="system-info">
          ...
          <div className="info-grid">
            ...
            <div>
              <strong>Node Version:</strong> {metrics?.system.nodeVersion}
            </div>
            <div>
              <strong>PID:</strong> {metrics?.system.pid}
            </div>
            <div>
              <strong>Uptime:</strong> {metrics?.server.uptime}
            </div>
          </div>
        </div>
      </div>
