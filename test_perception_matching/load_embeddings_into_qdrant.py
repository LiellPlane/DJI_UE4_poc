...
def get_embedding_size_from_first_pickle(embeddings_dir=None):
    """Get the embedding size from the first pickle file."""
    if embeddings_dir is None:
        current_file_dir = Path(__file__).parent
        repo_root = current_file_dir.parent
        embeddings_dir = repo_root / "TEST_PERCEPTION_MATCHING/embeddings_output_LiellVec"
    ...
    pickle_files = sorted(embeddings_dir.glob("**/*.pickle"))
    
    if not pickle_files:
        raise FileNotFoundError(f"No pickle files found in {embeddings_dir}")
    
    with open(pickle_files[0], 'rb') as f:
        embedding_results = pickle.load(f)
    
    if not isinstance(embedding_results, list) or not embedding_results:
        raise ValueError("Pickle file does not contain a list of embeddings")
    
    first_result = embedding_results[0]
    embedding = first_result.embedding.tolist() if hasattr(first_result.embedding, 'tolist') else first_result.embedding
...
def main():
    ...
    setup_collection(client, collection_name, vector_size=int(embedding_len))
